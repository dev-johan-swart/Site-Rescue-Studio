...
function routeEvidence(scanData) {
  const routes = Array.isArray(scanData?.routeHealth?.routes)
...
function linkEvidence(scanData) {
  ...
  const results = Array.isArray(scanData?.linkResults)
    ? scanData.linkResults
    : [];
...
function qualifyProspect(scanData) {
  ...
  if (!scanData || scanData.success === false) {
    return {
      priority: "failed",
      reason: "The website scan did not complete successfully.",
      evidence
    };
  }
  ...
  if (evidence.insecure) {
    highReasons.push("Confirmed HTTPS or mixed-content security problem.");
  }

  if (evidence.criticalBrokenRoutes > 0) {
    highReasons.push(
      `${evidence.criticalBrokenRoutes} confirmed customer-facing route(s) such as contact, booking or quote are broken/unreachable.`
    );
  }

  if (evidence.formFailure) {
    highReasons.push("A contact/enquiry form failure was detected.");
  }

  if (evidence.confirmedBrokenRoutes >= 2) {
    highReasons.push(
      `${evidence.confirmedBrokenRoutes} confirmed internal route failures were detected.`
    );
  }

  if (evidence.brokenLinks >= 3) {
    highReasons.push(
      `${evidence.brokenLinks} confirmed broken/unreachable links were detected.`
    );
  }
  ...
  if (evidence.canonicalMismatch) {
    moderateReasons.push("The public website and canonical hostname do not match.");
  }
