...
const PROVIDER_ORDER = ["websdocs", "openstreetmap"];
...
function openStreetMapQueriesForStage(stage, queries) {
  if (stage?.id !== "pretoria_centurion") return [];
  ...
  const mapped = requested.map(query => {
    ...
    return DEFAULT_DISCOVERY_QUERIES.find(item =>
...
async function discoverWithProviders({
  ...
  for(const provider of PROVIDER_ORDER){
    ...
    try{
      ...
      if (!Number(result?.candidateCount || 0)) {
        throw new Error(`${provider} returned no usable website candidates.`);
      }
...
module.exports={
  ...
  getDiscoveryStage,providerQueries,extractWebsDocsCandidates,discoverWebsDocs,openStreetMapQueriesForStage,discoverWithProviders
};
