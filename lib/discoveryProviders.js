...
const WISPRA_URL = "https://api.wispra.com/index/directory-businesses";
const PROVIDER_ORDER = ["openstreetmap", "websdocs", "wispra"];
...
function extractWispraCandidates(payload,limit){
  ...
  for(const row of rows){
    const website=row?.website||row?.websiteUrl||row?.url||"";
    const normalized=hostOf(website);
    if(!normalized||seen.has(normalized))continue;
    const country=String(row?.country||row?.countryName||row?.address?.country||"").toLowerCase();
    if(country && !country.includes("south africa") && country!=="za")continue;
    seen.add(normalized);
    candidates.push({
      website,normalized,
      name:String(row?.businessName||row?.name||"").trim()||null,
      city:String(row?.city||row?.address?.city||"").trim()||null,
...
async function discoverWispra({maxCandidates=20,fetchImpl=fetch}={}){
  const limit=Math.max(1,Math.min(Number(maxCandidates)||20,40));
  const params=new URLSearchParams({lang:"en",limit:String(limit)});
  const response=await fetchImpl(WISPRA_URL+"?"+params.toString(),{
    method:"GET",
    headers:{Accept:"application/json","User-Agent":"Site Rescue Studio Prospect Discovery/1.0 (site-rescue-studio.vercel.app)"}
  });
  let payload=null; try{payload=await response.json();}catch{}
  if(!response.ok){const error=new Error(payload?.message||`Wispra discovery returned HTTP ${response.status}.`);error.status=response.status;throw error;}
  const candidates=extractWispraCandidates(payload,limit);
  if(!candidates.length) throw new Error("Wispra returned no usable South African website candidates.");
  return {source:"wispra",candidates,queries:[{query:"South Africa fallback",websiteCandidatesAdded:candidates.length}],candidateCount:candidates.length};
}
    ...
    try{
      const result=provider==="openstreetmap"
        ...
        : provider==="websdocs"
          ...
          : await discoverWispra({maxCandidates});
...
module.exports={
  WEBS_DOCS_URL,WISPRA_URL,PROVIDER_ORDER,LOCAL_QUERIES,GAUTENG_QUERIES,SOUTH_AFRICA_QUERIES,DISCOVERY_STAGES,
  getDiscoveryStage,providerQueries,extractWebsDocsCandidates,discoverWebsDocs,extractWispraCandidates,discoverWispra,discoverWithProviders
};
