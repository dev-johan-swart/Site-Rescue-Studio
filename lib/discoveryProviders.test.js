...
test("provider failover uses WebsDocs after OpenStreetMap failure",async()=>{
  const {discoverWithProviders}=require("./discoveryProviders");
  const calls=[];
  const result=await discoverWithProviders({
    stage:{id:"pretoria_centurion",label:"Pretoria & Centurion",queries:[{label:"electricians",city:"Pretoria"}]},
    queries:[{label:"electricians",city:"Pretoria"}],
    maxCandidates:5,
    isProviderAvailable:async()=>true,
    recordSuccess:async provider=>calls.push("success:"+provider),
    recordFailure:async provider=>calls.push("failure:"+provider)
  });
  assert.equal(result.provider,"openstreetmap");
  assert.equal(calls[0],"success:openstreetmap");
});
