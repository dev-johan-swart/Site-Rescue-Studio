...
const OVERPASS_URLS = String(process.env.AUTOMATION_DISCOVERY_URLS || "")
  .split(",").map(value => value.trim()).filter(Boolean);

if (!OVERPASS_URLS.length) {
  OVERPASS_URLS.push(
...
async function searchOpenStreetMap(query, fetchImpl = fetch) {
  ...
  throw new Error(`All configured OpenStreetMap discovery servers failed. ${errors.join(" | ")}`);
}
