...
async function saveProspect(scanData) {
  ...
  /*
   * Full scan history is stored separately from the current
   * prospect summary. Existing installations receive the two
   * archive columns automatically on the next successful save.
   */
  await sql`
    ALTER TABLE scan_history
      ADD COLUMN IF NOT EXISTS scan_data JSONB,
      ADD COLUMN IF NOT EXISTS archive_token_hash TEXT;
  `;
  ...
  try {
    ...
    await sql`
      ...
      VALUES (
        ...
        ${scanData.keyProblems ?? null},
        ${scanData.potentialServices ?? null},
        ${JSON.stringify(scanData.archiveData || {})},
        ${archiveTokenHash}
      );
