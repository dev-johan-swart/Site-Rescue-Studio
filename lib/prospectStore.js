...
async function saveProspect(scanData) {
  ...
  try {
    await sql`ALTER TABLE scan_history ADD COLUMN IF NOT EXISTS scan_data JSONB, ADD COLUMN IF NOT EXISTS archive_token_hash TEXT;`;
  ...
  return {
    ...
    historyId:
      historySaved
        ? (await sql`SELECT id FROM scan_history WHERE archive_token_hash = ${archiveTokenHash} ORDER BY id DESC LIMIT 1;`)[0]?.id ?? null
        : null,

    archiveToken:
      historySaved ? archiveToken : null,
