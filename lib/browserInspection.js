...
const BROWSER_ROUTE_FETCH_TIMEOUT = 6000;
const BROWSER_ROUTE_CONCURRENCY = 5;
...
async function collectRenderedFormEmbedFailures(page) {
  try {
    return await page.evaluate(() => {
      ...
      const failurePattern =
        /(?:form\s+(?:is|was)\s+(?:missing|deleted|unavailable|not found|removed)|form[^.]{0,80}(?:could not|can't|cannot|unable to)\s+(?:be )?(?:loaded|displayed|found|accessed)|(?:oops|sorry)[^.]{0,80}form[^.]{0,40}(?:missing|unavailable|not found|deleted))/i;

      const failureMatch = bodyText.match(failurePattern);
      if (!failureMatch) return [];

      const providerPattern =
        /jotform|typeform|formstack|wufoo|formsite|123formbuilder|hubspot\s*forms?/i;

      const providerElements =
        Array.from(
          document.querySelectorAll(
            "iframe[src], script[src], [class], [id], [data-form-provider], [data-form]"
          )
        ).filter(visible);
        ...
        providerElements
          .map(element => {
            const values = [
              ...
              element.getAttribute("data-form-provider") || "",
              element.getAttribute("data-form") || ""
            ];
            return values.find(value =>
              providerPattern.test(value)
            ) || "";
          })
          .find(Boolean) || "";

      const providerFromText =
        bodyText.match(providerPattern)?.[0] || "";

      if (!providerEvidence && !providerFromText) return [];

      return [{
        type: "embedded-form-failure",
        provider:
          providerEvidence ||
          providerFromText ||
        ...
        confidence: "high",
        evidence: clean(
          [failureMatch[0], providerEvidence, providerFromText]
            .filter(Boolean)
            .join(" • ")
        ),
        url: window.location.href,
...
async function runBrowserInspection(url) {
  ...
  try {
    ...
    inspection.formEmbedFailures =
      await collectRenderedFormEmbedFailures(page);

    inspection.formReliability = [];

    const formProbes = new Map();
    const formProbeEntries = [];
    ...
    for (const form of inspection.renderedForms) {
      ...
      try {
        ...
        const key = parsed.href + "|" + formMethod;
        if (!formProbes.has(key)) {
          formProbes.set(key, null);
          formProbeEntries.push({
            key,
            action: parsed.href,
            method: formMethod
          });
        ...
        if (!formProbes.has(key)) {
          formProbes.set(key, {
            ...
            error: "Invalid form action URL."
          });
        }
      }
    }

    const probeAction = async entry => {
      ...
      try {
        ...
        return {
          ...
          finalUrl: response.url || entry.action,
          method: entry.method,
          probeMethod: "GET",
          verified:
            entry.method === "GET" && unsuccessfulProbe,
          reason:
            entry.method === "GET"
              ? unsuccessfulProbe
        ...
        return {
          action: entry.action,
          statusCode: null,
          finalUrl: entry.action,
          method: entry.method,
          probeMethod: "GET",
          verified: entry.method === "GET" && !timedOut,
          reason:
            entry.method === "GET"
              ? timedOut
                ? "get-endpoint-verification-timeout"
                : "get-endpoint-unreachable"
              : "submission-method-not-verified",
          error:
            timedOut
              ? "Automated form-endpoint verification timed out; the endpoint was not confirmed broken."
    ...
    for (let i = 0; i < formProbeEntries.length; i += 4) {
      const batch = formProbeEntries.slice(i, i + 4);
      const probeResults = await Promise.all(batch.map(probeAction));
      probeResults.forEach(result => formProbes.set(
        formProbeEntries.find(entry => entry.action === result.action && entry.method === result.method)?.key,
        result
      ));
    }

    inspection.formReliability = Array.from(formProbes.values()).filter(Boolean);
  ...
  console.log("[V3 BROWSER TIMING] Route discovery complete —", Date.now() - startedAt, "ms", "routes:", renderedInternalRoutes.length);

  inspection.routeHealth =
    await page.evaluate(
      async (routes, routeFetchTimeout, routeConcurrency) => {
        const results = [];
        ...
        const currentPageUrl = normalizeRoute(window.location.href);
        const testRoute = async route => {
          const startedAt = Date.now();
          if (normalizeRoute(route.href) === currentPageUrl) {
            return {
              url: route.href,
              ...
              responseTime: 0,
              anchorText: route.text,
              verificationSource: "browser-page-load"
            };
          }
          const controller = new AbortController();
          const timer = setTimeout(() => controller.abort(), routeFetchTimeout);
          try {
            ...
            return {
              ...
              statusCode: response.status,
              finalUrl,
              redirected: finalUrl !== route.href,
              redirectedOffOrigin: (() => {
                try {
                  return new URL(finalUrl).origin !== new URL(route.href).origin;
                } catch {
                  return false;
                }
              })(),
            ...
            return {
              url: route.href,
              path: new URL(route.href).pathname,
              status: timedOut ? "verification_timeout" : "unreachable",
              statusCode: null,
              finalUrl: route.href,
              redirected: false,
              redirectedOffOrigin: false,
              ...
              anchorText: route.text,
              error: timedOut
                ? "Automated direct-route verification timed out; the route was not confirmed broken."
                : error?.message || "Route could not be reached."
            };
          } finally {
            clearTimeout(timer);
          }
        };
        for (let i = 0; i < routes.length; i += routeConcurrency) {
          const batchResults = await Promise.all(
            routes.slice(i, i + routeConcurrency).map(testRoute)
          );
      ...
      renderedInternalRoutes,
      BROWSER_ROUTE_FETCH_TIMEOUT,
      BROWSER_ROUTE_CONCURRENCY
    );
  ...
  /*
    ...
    for (
      ...
      try {
        ...
        const routeEmbedFailures =
          await collectRenderedFormEmbedFailures(page);

        inspection.formEmbedFailures.push(
          ...routeEmbedFailures
        );
    ...
    console.log("[V3 BROWSER TIMING] Inspection complete —", Date.now() - startedAt, "ms");
