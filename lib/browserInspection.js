        ...
        forms =>
          forms.map(form => {
            ...
            const fields = controls.map(control => ({
              type: (control.getAttribute("type") || control.tagName.toLowerCase()).toLowerCase(),
            ...
            const text = (
              ...
              fields.map(f => [f.name,f.placeholder,f.ariaLabel].join(" ")).join(" ")
            ...
            const quote = /request.?a?.?quote|get.?a?.?quote|quote/i.test(text);
            const booking = /book|booking|appointment|schedule/i.test(text);
            const enquiry = /contact|enquir|reach.?us|get.?in.?touch|send.?message|request/i.test(text);
            const hasSubmitControl = Boolean(form.querySelector('button[type="submit"], input[type="submit"], button:not([type])'));
            const submitText = Array.from(form.querySelectorAll('button, input[type="submit"]')).map(e => (e.innerText || e.value || "").replace(/\s+/g," ").trim()).filter(Boolean).join(" ");
            ...
            else if(hasEmail && /subscribe|newsletter|updates/i.test(text)) type="newsletter";
            const contactIntent = !searchOnly && type!=="login" && type!=="newsletter" &&
              (hasMessage || hasName || hasPhone || hasEmail || quote || booking || enquiry);
            const confidence = (hasMessage && (hasEmail || hasPhone || hasName)) || quote || booking || (hasName && hasEmail)
              ? "high" : contactIntent ? "medium" : "low";
            return {
              source:"browser-rendered",type,confidence,contactIntent,
              action:form.action || null,
              method:(form.method || "get").toUpperCase(),
              fields:fields.length,inputCount:fields.length,
              fieldTypes:fields.map(f=>f.type),
              fieldLabels:fields.map(f=>f.name||f.placeholder||f.ariaLabel||"").filter(Boolean).slice(0,12),
              hasSubmit:hasSubmitControl,hasSubmitControl,
