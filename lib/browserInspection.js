...
Array.from(
  ...
  formFailures.forEach(form => {
    findings.push({
      id: "browser-form-reliability",
      title: "Contact form submission endpoint issue",
      description:
        form.statusCode
          ? `A detected form points to ${form.action}, which returned HTTP ${form.statusCode} when checked without submitting form data. The form itself was not submitted.`
          : `A detected form points to ${form.action}, but the submission endpoint could not be reached. The form itself was not submitted.`,
      status: "fail",
      severity: "high",
      evidence: form
    });
    ...
    for (const form of inspection.renderedForms) {
      ...
      try {
        ...
        inspection.formReliability.push({
          action: parsed.href,
          statusCode: null,
          reason: String(error?.message || "request failed"),
          method: form.method || "GET"
        });
      }
    }
