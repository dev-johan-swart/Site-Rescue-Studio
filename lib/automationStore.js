...
async function findPreviouslyScannedWebsite(sql, normalized) {
  const prospect = await sql`SELECT id, scan_count, last_scanned FROM prospects WHERE website_normalized = ${normalized} LIMIT 1`;
  if (prospect.length) return { type: "prospect", record: prospect[0] };
  const history = await sql`SELECT id, scanned_at FROM scan_history WHERE website_normalized = ${normalized} OR website_normalized LIKE ${normalized + "/%"} ORDER BY scanned_at DESC NULLS LAST, id DESC LIMIT 1`;
...
async function enqueueWebsites(websites, source = "manual") {
  ...
  for (const website of websites) {
    ...
    const existing = await sql`SELECT id, status FROM automation_queue WHERE website_normalized = ${normalized} AND status IN ('queued','running') LIMIT 1`;
    if (existing.length) { results.push({ website, status: "duplicate", queueId: existing[0].id }); continue; }
    const inserted = await sql`INSERT INTO automation_queue (website, website_normalized, source, status, scheduled_for) VALUES (${website}, ${normalized}, ${source}, 'queued', NOW()) RETURNING id, website, website_normalized, status`;
...
module.exports = {
  getSql, normalizeProspectWebsite, ensureAutomationSchema, enqueueWebsites,
  createRun, claimNextQueueItem, completeQueueItem, failQueueItem,
