...
async function enqueueWebsites(websites, source = "manual") {
  ...
  for (const website of websites) {
    ...
    if (!normalized) {
      results.push({ website, status: "invalid" });
      continue;
    }
    ...
    `;
    if (existing.length) {
      results.push({ website, status: "duplicate", queueId: existing[0].id });
      continue;
    }
    const inserted = await sql`
      INSERT INTO automation_queue (website, website_normalized, source, status, scheduled_for)
      VALUES (${website}, ${normalized}, ${source}, 'queued', NOW())
...
async function createRun(runKey) {
  const sql = getSql();
  await sql`
    INSERT INTO automation_runs (run_key, scheduled_for, status)
    VALUES (${runKey}, NOW(), 'queued')
    ON CONFLICT (run_key) DO NOTHING
  `;
  const rows = await sql`
    UPDATE automation_runs
    SET status = 'running', started_at = COALESCE(started_at, NOW()), updated_at = NOW()
    WHERE run_key = ${runKey} AND status <> 'running'
    RETURNING id, run_key, status
  `;
  if (rows.length) return { ...rows[0], acquired: true };
  const existing = await sql`
    SELECT id, run_key, status FROM automation_runs WHERE run_key = ${runKey} LIMIT 1
  `;
  return existing[0] ? { ...existing[0], acquired: false } : null;
}
...
module.exports = {
  ...
  addShortlistItem, addDueFollowUps, finishRun, recoverStaleQueueItems, markExhaustedFailures
};
