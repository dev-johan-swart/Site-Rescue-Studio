...
async function drainDiscoveryBacklog(sql, limit = 40) {
  ...
  const results = [];
  for (const row of rows) {
    const existing = await sql`SELECT id FROM automation_queue WHERE website_normalized = ${normalizeProspectWebsite(row.website)} AND status IN ('queued','running') LIMIT 1`;
...
module.exports = {
  getSql, normalizeProspectWebsite, historicalIdentityCandidates, getQueueDepth,
