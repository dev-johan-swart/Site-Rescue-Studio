...
async function getDailyScannedCount(sql) {
  const rows = await sql`
    SELECT COALESCE(SUM(scanned_count), 0)::int AS count
    FROM automation_runs
    WHERE started_at >= ((CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Johannesburg')::date AT TIME ZONE 'Africa/Johannesburg')
      AND started_at < (((CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Johannesburg')::date + 1) AT TIME ZONE 'Africa/Johannesburg')
