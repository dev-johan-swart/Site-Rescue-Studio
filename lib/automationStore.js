...
async function getDailyScannedCount(sql, cycleDate = null) {
  // Count durable queue completions inside the same SAST 08:00 -> 08:00
  // automation cycle used by claimNextDailyRun(). This deliberately does NOT
  // reset at South African midnight, because one overnight cycle spans two
  // calendar dates.
  const rows = cycleDate
    ? await sql`
        ...
        WHERE status = 'completed'
          AND completed_at >= (${String(cycleDate)}::date + TIME '08:00') AT TIME ZONE 'Africa/Johannesburg'
          AND completed_at < ((${String(cycleDate)}::date + 1) + TIME '08:00') AT TIME ZONE 'Africa/Johannesburg'
      `
    : await sql`
        WITH cycle AS (
          SELECT CASE
        ...
        SELECT COUNT(*)::int AS count
        FROM automation_queue, cycle
        WHERE status = 'completed'
          AND completed_at >= (cycle_date + TIME '08:00') AT TIME ZONE 'Africa/Johannesburg'
          AND completed_at < ((cycle_date + 1) + TIME '08:00') AT TIME ZONE 'Africa/Johannesburg'
      `;
  return Number(rows[0]?.count || 0);
}
...
async function claimNextDailyRun(sql, cycleDate, maxBatches = 5) {
...
async function reserveDiscoveryProvider(sql, provider, dailyLimit = 2, cycleDate = null) {
  // Discovery quotas follow the same SAST automation cycle as the worker,
  // rather than UTC midnight. This prevents quota resets during an overnight run.
...
async function drainDiscoveryBacklog(sql, limit = 80) {
  ...
  for (const row of rows) {
    ...
    results.push({ id: row.id, website: row.website });
  }
  return results;
}
...
async function markDiscoveryBacklogResults(sql, items, enqueueResults) {
  ...
  for (const item of items || []) {
    ...
    if (status === 'queued') {
      await sql`UPDATE automation_discovery_backlog SET status = 'queued', queued_at = NOW() WHERE id = ${item.id}`;
...
module.exports = {
  getSql, normalizeProspectWebsite, historicalIdentityCandidates, getQueueDepth, getDailyScannedCount, recoverStaleAutomationRuns, claimNextDailyRun,
  getDiscoveryProviderState, reserveDiscoveryProvider, recordDiscoveryProviderSuccess, recordDiscoveryProviderFailure,
  getDiscoveryStageState, setDiscoveryStageState, addDiscoveryBacklogCandidates, drainDiscoveryBacklog, markDiscoveryBacklogResults,
