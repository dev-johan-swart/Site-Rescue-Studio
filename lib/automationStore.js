...
async function getDailyScannedCount(sql) {
  // Count durable queue completions, not run counters. A function can be
  // terminated after a scan succeeds but before finishRun() persists its
  // aggregate counters. Queue completion is the durable source of truth.
  const rows = await sql`
    SELECT COUNT(*)::int AS count
    FROM automation_queue
    WHERE status = 'completed'
      AND completed_at >= ((CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Johannesburg')::date AT TIME ZONE 'Africa/Johannesburg')
      AND completed_at < (((CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Johannesburg')::date + 1) AT TIME ZONE 'Africa/Johannesburg')
  `;
  return Number(rows[0]?.count || 0);
}
...
async function claimNextDailyRun(sql, cycleDate, maxBatches = 5) {
  ...
  const limit = Math.max(1, Math.min(Number(maxBatches) || 5, 5));
  const rows = await sql`
    WITH candidate AS (
